...
if (@($parsed.SuggestedValidation) -notcontains "scripts\check-smoke-build-target-preflight.bat -Stage generate-project -Json -SummaryOnly") {
	throw "ecosystem agent JSON output did not include the compact smoke-build preflight validation command."
}
