...
set "SCRIPT_DIR=%~dp0"
set "FIRST_RUN_PS1=%SCRIPT_DIR%first-run.ps1"

where pwsh.exe >nul 2>nul
if errorlevel 1 goto try_windows_powershell
pwsh.exe -NoProfile -ExecutionPolicy Bypass -File "%FIRST_RUN_PS1%" %*
exit /b %ERRORLEVEL%

:try_windows_powershell
where powershell.exe >nul 2>nul
if errorlevel 1 goto powershell_missing
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%FIRST_RUN_PS1%" %*
exit /b %ERRORLEVEL%
