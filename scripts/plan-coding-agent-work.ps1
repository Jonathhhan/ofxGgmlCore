...
if ($tasks.Count -eq 0 -and $managedReady.Count -gt 0) {
	...
	if ($core.Count -gt 0) {
		$tasks.Add((New-AgentTask `
			...
			-Validation "scripts/check-ecosystem-readiness.bat -SkipDoctorTests; scripts/check-ecosystem-readiness.bat -SkipDoctorTests -Json -SummaryOnly; scripts/plan-ecosystem.bat -Json -SummaryOnly; scripts/test-workflow-status-report.ps1; scripts/test-release-readiness-score.ps1; scripts/test-release-readiness-plan.ps1; scripts/test-smoke-build-ci-report.ps1; scripts/test-agent-branch-cleanup.ps1; scripts/plan-agent-branch-cleanup.bat -Json -SummaryOnly; scripts/plan-smoke-build-target-handoff.bat -Stage generate-project; scripts/check-smoke-build-target-preflight.bat -Stage generate-project; scripts/check-smoke-build-target-postflight.bat -Stage generate-project; scripts/plan-smoke-build-project-repair.bat -Stage verify-generated-project; scripts/plan-smoke-build-compile.bat -Stage compile-example; scripts/build-smoke-example.bat -Repository ofxGgmlSam -Example ofxGgmlSamPointExample"))
	}
