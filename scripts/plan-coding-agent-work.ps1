...
if ($tasks.Count -eq 0 -and $managedReady.Count -gt 0) {
	...
	if ($core.Count -gt 0) {
		$tasks.Add((New-AgentTask `
			...
			-Validation "scripts/check-ecosystem-readiness.bat -SkipDoctorTests; scripts/check-ecosystem-readiness.bat -SkipDoctorTests -Json -SummaryOnly; scripts/status-family.bat -Json -SummaryOnly; scripts/test-family-status.ps1; scripts/audit-ecosystem.bat -Strict -Json -SummaryOnly; scripts/test-ecosystem-audit.ps1; scripts/plan-doctor-rollout.bat -Json -SummaryOnly; scripts/test-doctor-rollout-plan.ps1; scripts/plan-ecosystem.bat -Json -SummaryOnly; scripts/test-workflow-status-report.ps1; scripts/test-release-readiness-score.ps1; scripts/test-release-readiness-plan.ps1; scripts/test-smoke-build-ci-report.ps1; scripts/test-agent-branch-cleanup.ps1; scripts/plan-agent-branch-cleanup.bat -Json -SummaryOnly; scripts/plan-smoke-build-target-handoff.bat -Stage generate-project -Json -SummaryOnly; scripts/check-smoke-build-target-preflight.bat -Stage generate-project -Json -SummaryOnly; scripts/check-smoke-build-target-postflight.bat -Stage generate-project -Json -SummaryOnly; scripts/plan-smoke-build-project-repair.bat -Stage verify-generated-project; scripts/plan-smoke-build-compile.bat -Stage compile-example; scripts/build-smoke-example.bat -Repository ofxGgmlSam -Example ofxGgmlSamPointExample"))
	}
