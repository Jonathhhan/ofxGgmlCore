...
`ofxGgmlEmbeddingExample` uses `ofxImGui` for an editable input field, embedding
server settings, request status, and a compact preview of the returned vector.
The run script starts a dedicated `llama-server --embeddings` process on `8081`
when one is not already reachable. Use an embedding-tuned GGUF for meaningful
vectors; general instruct models are only useful for smoke testing.
