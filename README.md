...
`ofxGgmlTextExample` reads `OFXGGML_LLAMA_CLI` and `OFXGGML_TEXT_MODEL` from the
environment, then runs one llama.cpp CLI prompt through `ofxGgmlTextGenerator`.
The request runs on a worker thread so the window paints immediately; press `R`
to run again or `C` to cancel after the next output chunk. Missing or invalid
environment paths are shown in the example window.

You can also launch it with local paths without setting global environment
variables:

```powershell
scripts\run-text-example.bat -LlamaCli C:\path\to\llama-cli.exe -Model C:\path\to\model.gguf
scripts\run-text-example.ps1 -LlamaCli C:\path\to\llama-cli.exe -Model C:\path\to\model.gguf
```

If the paths are omitted, the example and launcher look for `llama-cli` beside
the app and under local `bin`, `data`, `tools`, and `libs/llama.cpp/build/bin`
folders. They look for the first `.gguf` model under local `data/models` or
`models` folders.
