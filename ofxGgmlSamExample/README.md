...
The example now targets `sam3.cpp`, which supports SAM 2, SAM 2.1, SAM 3, and EdgeTAM GGML models. On Windows with CUDA Toolkit and Visual Studio CUDA integration installed:
...
Place a converted model in:

```text
bin/data/models/sam3/model.gguf
```

or any `.gguf`, `.ggml`, or `.bin` file under `bin/data/models/sam3`. The example scans that directory and uses the first model file it finds.

To opt into the real backend, define `OFXGGML_ENABLE_SAM3_ADAPTER=1`, add `libs/sam3.cpp` and `libs/sam3.cpp/stb` to the include paths, and link the matching `sam3.lib` plus the `ggml` libraries from either `libs/sam3.cpp/build-cuda` or `libs/sam3.cpp/build-cpu`. Without that macro/link setup the app uses the deterministic preview backend, so the example stays buildable on machines that have not installed `sam3.cpp`.
...
The checkout is not compiled automatically because the pinned `sam.cpp` source uses older ggml allocator APIs than the addon-local ggml `.11.0` build. New work should prefer the `sam3.cpp` adapter above.
